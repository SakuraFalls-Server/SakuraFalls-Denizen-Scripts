friends_command_friend:
    ...
    - choose <context.args.first.to_lowercase>:
        ...
        # ACCEPT FRIEND
            ...
            - if <[target]> == null:
                - narrate format:formats_prefix "<&c>That player is not online."
                - stop
        ...
        # MESSAGE ADDED FRIENDS
            ...
            - define target <server.match_player[<context.args.get[2]>].if_null[null]>
            - if <[target]> == null:
                - narrate format:formats_prefix "<&c>That friend is not online."
                - stop
            - define message <context.args.get[3].to[last].space_separated>
            - run friends_message def.player:<player> def.target:<[target]> def.message:<[message]>
        - case reply r:
        # REPLY TO FRIENDS THAT MESSAGE YOU
            - if <context.args.size> < 2:
        ...
        - case wipe:
        # WIPE ALL FRIENDS (prompts a confirm message after)
        ...
        - case confirm:
        # CONFIRM WIPE (for wipe all friends)
            - if !<player.has_flag[friends_wipe_confirm]>:
