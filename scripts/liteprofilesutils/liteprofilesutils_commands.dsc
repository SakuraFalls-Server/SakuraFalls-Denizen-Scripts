...
liteprofilesutils_command_findprofiles:
    debug: false
    type: command
    name: findprofiles
    description: Find profiles of by player name.
    usage: /findprofiles (player)
    tab completions:
        1: <server.online_players.parse[name]>
    permission: liteprofilesutils.command.findprofiles
    script:
    - if <context.source_type> != player:
        - narrate "<&c>Run this command as a player"
        - stop
